...
class _WatchScreenState extends State<WatchScreen>
  ...
  void _goToEpisode(int index) {
    ...
    debugPrint(
      ...
      'episode=${widget.anime.episodes[index].number}',
    );
    setState(() {
      _currentIndex = index;
      _isPlaying = false;
  ...
  void _startPlaybackTimer() {
    ...
    _playbackTimer = Timer.periodic(const Duration(seconds: 1), (timer) {
      ...
      final next = (_seekValue + (1 / _episodeDurationSeconds)).clamp(0.0, 1.0);
      setState(() => _seekValue = next);
      debugPrint(
        '[WatchScreen] timer tick anime.id="${widget.anime.id}" episode=${_currentEpisode.number} '
        'watchProgress=${_seekValue.toStringAsFixed(4)}',
      );
      debugPrint('[CW] timer tick -> _syncContinueWatching');
      _syncContinueWatching();
  ...
  void _syncContinueWatching() {
    debugPrint('[CW] progress update -> calling updateContinueWatching');
    debugPrint(
      '[WatchScreen] watchProgress update anime.id="${widget.anime.id}" '
      'episode=${_currentEpisode.number} progress=${_seekValue.toStringAsFixed(4)}',
    );
