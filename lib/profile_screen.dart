...
class _ProfileScreenState extends State<ProfileScreen>
  ...
  String? _previewFxId;
  String? _previewBgId;
  bool get _isPreviewMode => _previewFrameId != null || _previewDecoId != null || _previewFxId != null || _previewBgId != null;
                        ...
                        slivers: [
                          ...
                          SliverToBoxAdapter(
                            child: Column(children: [
                              ...
                              Padding(
                                padding: const EdgeInsets.symmetric(horizontal: 16),
                                child: _ProfileCosmeticsShowcase(
                              ...
                              Padding(
                                ...
                                child: StaggeredGrid.count(
                                  ...
                                  children: [
                                    ...
                                    // Row 5: AI Roast — full width
                                    const StaggeredGridTile.count(
                                      crossAxisCellCount: 4,
                                      mainAxisCellCount: 2.4,
                                      child: _AiRoastCard(),
                                    ),
                                    // Row 6: Share Profile — full width, short
                                    const StaggeredGridTile.count(
                                      crossAxisCellCount: 4,
                                      mainAxisCellCount: 1.5,
...
class _ProfileCosmeticsShowcase extends StatefulWidget {
  ...
  final Uint8List? avatarBytes;
  const _ProfileCosmeticsShowcase({
    required this.equippedFrameId, required this.equippedBgId, required this.equippedFxId,
    required this.equippedDecoId, required this.onEquip, this.onPreviewChanged, this.avatarBytes,
  });
  @override State<_ProfileCosmeticsShowcase> createState() => _ProfileCosmeticsShowcaseState();
}

class _ProfileCosmeticsShowcaseState extends State<_ProfileCosmeticsShowcase> {
  late List<_CosmeticItem> _items;
  int _previewIndex = 0;
  ...
  void _selectIndex(int i) {
    ...
    final item = _items[i];
    // Only preview supported types — BG now included
    if (['Avatar Border', 'Avatar Deco', 'Profile FX', 'Profile BG'].contains(item.type)) {
      widget.onPreviewChanged?.call(item.type, item.id);
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    final ownedCount = _items.where((i) => i.owned).length;
    final isEquipped = (item.type == 'Avatar Border' && widget.equippedFrameId == item.id) ||
                       (item.type == 'Profile FX' && widget.equippedFxId == item.id) ||
                       (item.type == 'Avatar Deco' && widget.equippedDecoId == item.id);
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Row(children: [
          Container(
            ...
            decoration: BoxDecoration(
              gradient: LinearGradient(colors: [AppTheme.highlight.withOpacity(0.18), AppTheme.accent.withOpacity(0.10)]),
              borderRadius: BorderRadius.circular(8),
              border: Border.all(color: AppTheme.highlight.withOpacity(0.40)),
            ),
            child: Row(mainAxisSize: MainAxisSize.min, children: [
              Icon(Icons.diamond_rounded, color: AppTheme.highlight, size: 12),
              const SizedBox(width: 5),
              Text('COSMETIC SHOWROOM', style: TextStyle(color: AppTheme.textPrimary, fontSize: 10, fontWeight: FontWeight.w900, letterSpacing: 0.8)),
            ]),
          ),
          const Spacer(),
          Container(
            padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
            decoration: BoxDecoration(color: AppTheme.highlight.withOpacity(0.08), borderRadius: BorderRadius.circular(8)),
            child: Text('$ownedCount/${_items.length} Owned  •  Shop ›', style: TextStyle(color: AppTheme.highlight, fontSize: 10, fontWeight: FontWeight.w900)),
          ),
        ]),
        const SizedBox(height: 14),

        // ── HERO SHOWCASE — big avatar + item spotlight ──
        AnimatedContainer(
          duration: const Duration(milliseconds: 350),
          ...
          decoration: BoxDecoration(
            ...
            gradient: LinearGradient(
              begin: Alignment.topLeft, end: Alignment.bottomRight,
              colors: [item.color.withOpacity(0.22), AppTheme.surface, item.color.withOpacity(0.10)],
            ),
            border: Border.all(color: item.color.withOpacity(0.55), width: 1.8),
            boxShadow: [BoxShadow(color: item.color.withOpacity(0.28), blurRadius: 28, spreadRadius: -2, offset: const Offset(0, 8))],
          ),
          child: Padding(
            padding: const EdgeInsets.all(16),
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Row(crossAxisAlignment: CrossAxisAlignment.center, children: [
                ...
                Stack(alignment: Alignment.center, children: [
                  // Multi-layer rarity glow
                  Container(
                    width: 106, height: 106,
                    decoration: BoxDecoration(
                      shape: BoxShape.circle,
                      boxShadow: [
                        BoxShadow(color: item.color.withOpacity(0.55), blurRadius: 32, spreadRadius: 6),
                        BoxShadow(color: item.color.withOpacity(0.25), blurRadius: 60, spreadRadius: 12),
                      ],
                    ),
                  ),
                  // Outer ring — PNG asset border
                  _PngBorderRing(
                    frameId: item.type == 'Avatar Border' ? item.id : widget.equippedFrameId,
                    size: 106,
                  ),
                  // Avatar — 88px, much more prominent
                  Container(
                    width: 80, height: 80,
                    decoration: BoxDecoration(
                      shape: BoxShape.circle,
                      border: Border.all(color: item.color, width: 2.5),
                      boxShadow: [BoxShadow(color: item.color.withOpacity(0.4), blurRadius: 16, spreadRadius: 2)],
                    ),
                    child: ClipOval(child: widget.avatarBytes != null
                      ? Image.memory(widget.avatarBytes!, fit: BoxFit.cover, errorBuilder: (_,__,___) => _avatarFallback())
                      : Image.asset('asset/Hitaku Avatar Pack.png', fit: BoxFit.cover, errorBuilder: (_,__,___) => _avatarFallback())),
                  ),
                  // Rarity badge — repositioned to bottom-right
                  Positioned(
                    bottom: 2, right: 2,
                    child: Container(
                      ...
                      decoration: BoxDecoration(
                        ...
                        boxShadow: [BoxShadow(color: item.color.withOpacity(0.7), blurRadius: 10)],
                      ),
                      child: Icon(
                        item.rarity == 'LEGENDARY' ? Icons.auto_awesome
                          : item.rarity == 'MYSTIC' ? Icons.bolt_rounded
                          : Icons.star_rounded,
                ...
                Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Row(children: [
                    _RarityBadge(rarity: item.rarity, color: item.color),
                    const SizedBox(width: 6),
                    Text(item.type.toUpperCase(), style: TextStyle(color: AppTheme.textSecondary, fontSize: 8, fontWeight: FontWeight.w900, letterSpacing: 0.5)),
                  ]),
                  const SizedBox(height: 5),
                  Text(item.name, style: TextStyle(color: AppTheme.textPrimary, fontSize: 17, fontWeight: FontWeight.w900, letterSpacing: -0.4, height: 1.1)),
                  const SizedBox(height: 6),
                  // Social proof with avatar stack
                  Row(children: [
                    // Mini avatar stack (decorative circles)
                    SizedBox(
                      width: 42, height: 16,
                      child: Stack(children: [
                        for (int s = 0; s < 3; s++)
                          Positioned(
                            ...
                            child: Container(
                              ...
                              decoration: BoxDecoration(
                                shape: BoxShape.circle,
                                color: [item.color, AppTheme.highlight, AppTheme.accent][s].withOpacity(0.8),
                                border: Border.all(color: AppTheme.background, width: 1.5),
                              ),
                              child: Center(child: Icon(Icons.person_rounded, color: Colors.white, size: 9)),
                            ),
                          ),
                      ]),
                    ),
                    const SizedBox(width: 6),
                    Text('${(item.id.hashCode.abs() % 800) + 200} orang pakai',
                      style: TextStyle(color: AppTheme.textSecondary, fontSize: 9.5, fontWeight: FontWeight.w700)),
                  ]),
                ])),
              ]),
              ...
              // ── Action button — full width, high contrast ──
              if (isEquipped)
                Container(
                  width: double.infinity,
                  ...
                  decoration: BoxDecoration(
                    ...
                    boxShadow: [BoxShadow(color: const Color(0xFF4CAF50).withOpacity(0.30), blurRadius: 14, offset: const Offset(0, 4))],
                  ),
                  child: const Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                    Icon(Icons.check_circle_rounded, color: Color(0xFF81C784), size: 16),
                    SizedBox(width: 6),
                    Text('EQUIPPED & ACTIVE', style: TextStyle(color: Color(0xFF81C784), fontSize: 11, fontWeight: FontWeight.w900, letterSpacing: 0.5)),
                  ]),
                )
              else if (item.owned)
                GestureDetector(
                  onTap: () => widget.onEquip(item.type, item.name),
                  child: Container(
                    width: double.infinity,
                    padding: const EdgeInsets.symmetric(vertical: 13),
                    decoration: BoxDecoration(
                      gradient: const LinearGradient(colors: [Color(0xFFD4AF37), Color(0xFFC17E74)]),
                      borderRadius: BorderRadius.circular(14),
                      boxShadow: [
                        BoxShadow(color: const Color(0xFFD4AF37).withOpacity(0.50), blurRadius: 18, offset: const Offset(0, 5)),
                        BoxShadow(color: const Color(0xFFD4AF37).withOpacity(0.20), blurRadius: 36, offset: const Offset(0, 10)),
                      ],
                    ),
                    child: const Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                      Icon(Icons.bolt_rounded, color: Colors.white, size: 18),
                      SizedBox(width: 6),
                      Text('EQUIP SEKARANG', style: TextStyle(color: Colors.white, fontSize: 12, fontWeight: FontWeight.w900, letterSpacing: 0.6)),
                    ]),
                  ),
                )
              else
                GestureDetector(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      ...
                      borderRadius: BorderRadius.circular(14),
                      boxShadow: [
                        BoxShadow(color: const Color(0xFFFF8F00).withOpacity(0.50), blurRadius: 18, offset: const Offset(0, 5)),
                        BoxShadow(color: const Color(0xFFFF8F00).withOpacity(0.20), blurRadius: 36, offset: const Offset(0, 10)),
                      ],
                    ),
                    child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                      const Icon(Icons.shopping_bag_rounded, color: Colors.white, size: 16),
                      const SizedBox(width: 6),
                      Text('BELI — ${item.price}', style: const TextStyle(color: Colors.white, fontSize: 12, fontWeight: FontWeight.w900, letterSpacing: 0.4)),
                    ]),
                  ),
                ),
            ]),
        ...
        SizedBox(
          ...
          child: ListView.separated(
            ...
            itemBuilder: (_, i) {
              final it = _items[i];
              final sel = i == _previewIndex;
              final eq = (it.type == 'Avatar Border' && widget.equippedFrameId == it.name) ||
                         (it.type == 'Profile FX' && widget.equippedFxId == it.name) ||
                         (it.type == 'Avatar Deco' && widget.equippedDecoId == it.name);
              return GestureDetector(
                ...
                child: AnimatedContainer(
                  ...
                  decoration: BoxDecoration(
                    borderRadius: BorderRadius.circular(16),
                    gradient: sel
                      ? LinearGradient(begin: Alignment.topLeft, end: Alignment.bottomRight, colors: [it.color.withOpacity(0.18), AppTheme.surface])
                      : null,
                    color: sel ? null : AppTheme.surface.withOpacity(0.6),
                    border: Border.all(color: sel ? it.color : AppTheme.surfaceElevated, width: sel ? 2 : 1),
                    boxShadow: sel ? [BoxShadow(color: it.color.withOpacity(0.30), blurRadius: 14, spreadRadius: -2)] : [],
                  ),
                  child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                    // Thumbnail: mini avatar with ring for border items, icon for others
                    SizedBox(
                      height: 52, width: double.infinity,
                      child: Center(
                        child: it.type == 'Avatar Border'
                          ? Stack(alignment: Alignment.center, children: [
                              Container(
                                width: 52, height: 52,
                                decoration: BoxDecoration(shape: BoxShape.circle,
                                  boxShadow: [BoxShadow(color: it.color.withOpacity(sel ? 0.45 : 0.20), blurRadius: 12)]),
                              ),
                              _PngBorderRing(frameId: it.id, size: 52),
                              Container(
                                width: 38, height: 38,
                                decoration: BoxDecoration(shape: BoxShape.circle,
                                  color: it.color.withOpacity(0.15),
                                  border: Border.all(color: it.color.withOpacity(0.5), width: 1.5)),
                                child: ClipOval(child: widget.avatarBytes != null
                                  ? Image.memory(widget.avatarBytes!, fit: BoxFit.cover, errorBuilder: (_,__,___) => _avatarFallback())
                                  : Image.asset('asset/Hitaku Avatar Pack.png', fit: BoxFit.cover, errorBuilder: (_,__,___) => _avatarFallback())),
                              ),
                          ...
                          : Container(
                              ...
                              decoration: BoxDecoration(
                                borderRadius: BorderRadius.circular(12),
                                color: it.color.withOpacity(sel ? 0.20 : 0.08),
                                border: Border.all(color: it.color.withOpacity(sel ? 0.5 : 0.15)),
                                boxShadow: sel ? [BoxShadow(color: it.color.withOpacity(0.30), blurRadius: 10)] : [],
                              ),
                              child: Center(child: Icon(it.icon, color: it.color, size: sel ? 26 : 22)),
                            ),
                      ),
                    ),
                    const SizedBox(height: 6),
                    Text(it.name, maxLines: 1, overflow: TextOverflow.ellipsis,
                      style: TextStyle(color: sel ? AppTheme.textPrimary : AppTheme.textSecondary, fontSize: 10, fontWeight: FontWeight.w900)),
                    const SizedBox(height: 4),
                    // Status chip
                    Container(
                      width: double.infinity, padding: const EdgeInsets.symmetric(vertical: 3.5),
                      decoration: BoxDecoration(
                        color: eq ? AppTheme.success.withOpacity(0.12) : (it.owned ? it.color.withOpacity(0.10) : const Color(0xFFFFB300).withOpacity(0.10)),
                        borderRadius: BorderRadius.circular(6),
                        border: Border.all(color: eq ? AppTheme.success.withOpacity(0.35) : (it.owned ? it.color.withOpacity(0.25) : const Color(0xFFFFB300).withOpacity(0.25))),
                      ),
                      child: Center(child: Text(
                        eq ? '✓ Active' : (it.owned ? 'Owned' : it.price.replaceAll(' Gems', 'G')),
                        style: TextStyle(color: eq ? AppTheme.success : (it.owned ? it.color : const Color(0xFFFF8F00)), fontSize: 8, fontWeight: FontWeight.w900),
                      )),
                    ),
                  ]),
                ),
  ...
  Widget _badge(Color c, IconData icon, String label) => Container(
    padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 5),
    ...
    child: Row(mainAxisSize: MainAxisSize.min, children: [Icon(icon, color: c, size: 10), const SizedBox(width: 3), Text(label, style: TextStyle(color: c, fontSize: 8.5, fontWeight: FontWeight.w900))]),
  );

  Widget _actionBtn(Gradient g, String label) => Container(
    padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 6),
    decoration: BoxDecoration(gradient: g, borderRadius: BorderRadius.circular(10)),
    child: Text(label, style: const TextStyle(color: Colors.white, fontSize: 9.5, fontWeight: FontWeight.w900)),
  );

  void _buy(_CosmeticItem item) {
    final idx = _items.indexOf(item);
    showDialog(context: context, builder: (_) => AlertDialog(
      backgroundColor: AppTheme.surfaceElevated,
      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20), side: BorderSide(color: AppTheme.accent.withOpacity(0.25))),
      title: Text('Konfirmasi Pembelian', style: TextStyle(color: AppTheme.textPrimary, fontWeight: FontWeight.w900)),
      content: Text('Beli "${item.name}" seharga ${item.price}?', style: TextStyle(color: AppTheme.textSecondary)),
      actions: [
        TextButton(onPressed: () => Navigator.pop(context), child: Text('Batal', style: TextStyle(color: AppTheme.textSecondary))),
        ElevatedButton(
          onPressed: () { Navigator.pop(context); setState(() => _items[idx] = item.copyWith(owned: true)); },
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Row(children: [
          ...
          Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text('jam ditonton', style: TextStyle(color: AppTheme.textSecondary, fontSize: 11)),
            Text('≈ 595 hari', style: TextStyle(color: AppTheme.textSecondary, fontSize: 10)),
          ]),
        ]),
        const SizedBox(height: 12),
        Row(children: [
